...
class TeamMember4 {
    ...
    public TeamMember4(String name, int qualityOfWork, int productivity, int communication, int hoursPerWeek, boolean hasChildren, int yearsWorking, int age) {
        this.name = name;
...
public class TeamPerformanceManager4 {
    ...
    public static List<TeamMember> calculateBonus(List<TeamMember> team) {
        ...
        for (TeamMember member : team) {
            if (member.isEligibleForBonus()) {
                ...
                int bonusAmount = member.calculateBonusAmount();
                totalBonusAmount += bonusAmount;
                totalBonusEligibleMembers++;

                if (bonusAmount < smallestBonus) {
                    smallestBonus = bonusAmount;
                    memberWithSmallestBonus = member;
                }

                if (bonusAmount > biggestBonus) {
                    biggestBonus = bonusAmount;
    ...
    public static void main(String[] args) {
        ...
        // Print out data of member with the smallest bonus
        if (memberWithSmallestBonus != null) {
            System.out.println("Member with biggest bonus:");
            System.out.println("Name: " + memberWithSmallestBonus.getName());
            System.out.println("Bonus: " + memberWithSmallestBonus.getBonus());
            System.out.println("BonusAmount: " + memberWithSmallestBonus.getBonusAmount());
            System.out.println("BonusAmount: " + memberWithSmallestBonus.getBonusAmount());
        } else {
            System.out.println("No member eligible for bonus.");
        }
    ...
    // Variables to bonus-eligible members, bonus amount, biggest bonus, and smallest bonus
    private static int totalBonusEligibleMembers = 0;
    private static int totalBonusAmount = 0;
    private static int biggestBonus = 0;
    private static int smallestBonus = 0;
    private static TeamMember memberWithBiggestBonus = null;
    private static TeamMember memberWithSmallestBonus = null;
